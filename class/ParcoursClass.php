...
//********************************************************
    ...
    public function add_balise($id_balise, $ordre = 0, $nom = "", $value = 1) {
        if ($this->_id > 0) {
    ...
    public function remove_balise($list_balise) {
        if ($this->_id > 0) {
            if (is_array($list_balise)) {
                foreach ($list_balise as $id_balise) {
    ...
    public function set_ordre($ordre=false){
        if ($this->_id > 0) {
            if($ordre==2){
                $result = $this->_db->query("SELECT id from liste_balises WHERE id_parcours = '" . $this->_id . "'");
                if ($result->num_rows > 0) {
                    $i = 0;
                    foreach ($result as $r){
                        $i++;
                        $this->_db->query("UPDATE liste_balises SET ordre = '" . $i . "' WHERE id = '" . $r['id'] . "'");
                    }
                }
            }else if($ordre==1){
                $this->_db->query("UPDATE liste_balises SET ordre = '0' WHERE id_parcours = '" . $this->_id . "'");
            }
        }
    }
    ...
    public function get_last_parcours($id_activite,$last_index) {
        if ($id_activite > 0) {
            $result = $this->_db->query("SELECT id,nom FROM parcours WHERE parcours.id > '".$last_index."'  ORDER BY NOM");
            if ($result->num_rows > 0) {
                return $result;
            }
        }
    ...
    public function get_all_datas($id_activite) {
        if ($id_activite > 0) {
            $result = $this->_db->query("SELECT t_start,t_end,co_datas.id, co.id as id_co, co.id_parcours,tag,temps,co.id_participant "
                    . "FROM co_datas,co "
                    . "WHERE co.id_activite = '".$id_activite."' "
                    . "AND co_datas.id_co = co.id");
            if ($result->num_rows > 0) {
                return $result;
            }
